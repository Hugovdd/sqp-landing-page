...
`wrangler.jsonc` sets `workers_dev: false` / `preview_urls: false` so production
is only reachable via its custom domain (no duplicate `*.workers.dev` URL).
Staging's workers.dev URL is its only URL, so `deploy:staging` flips those flags
back via [`scripts/enable-workers-dev.mjs`](../scripts/enable-workers-dev.mjs)
before deploying.
